...
public class StudentManager implements Program {
	...
	public void load(String fileName) {
		try(FileInputStream fis = new FileInputStream(fileName);
				...
				sublist = (List<String>)ois.readObject();
			} catch (Exception e) {
			} 
		}
		
	/**
	 * 콘솔에서 정수를 입력받아 반환하는 메소드로 잘못 입력하면 (문자열) 정수의 가장 작을 수를 반환
	...
	/**학년 반 번호 이름를 입력받아 학생 객체를 만들어서 반환하는 메소드
	 * @return 학년, 반, 번호, 이름을 받아 만들어진 학생 객체 
	 */
	...
	private void runStudentUpdateMenu(int menu) {
		...
		private void studentInfoUpdate() {
			//수정하려는 학년 반 번호를 입력하고
			//입력한 정보를 이용해서 객체를 생성
			//생성하는 이유 : =>indexOf 또는 Contains등을 이용해서 객체를 쉽게 비교하기 위해서
			Student std = inputStudent();//위의 2 요구사항을 한번에 해결
			//생성한 객체를 이용해서 리스트에 몇번지에 있는지 번지를 가져옴
			...
			//else로 처리하지 않아도 됨 return으로 끝난 코드임
			...
			//=>번지를 이용하여 삭제하면 삭제된 객체를 반환
			std = list.remove(index);	
			//생성한 객체가 리스트에 있는 지 확인해서 있으면 알림문구 출력 후 종료
			...
			//삭제된 객체의 update메소드를 이용해서 학년, 반, 번호, 이름을 수정
		...
		private void insertSubejectScore() {
			//등록된 과목이 없으면 알림문구 출력 후 종료
			if(sublist.size() == 0) {
				System.out.println("등록된 과목이 없어서 추가할 수 없습니다, 과목을 등록하세요");
				return;
			}
			...
			if(!sublist.contains(subject.getName())) {
			...
			if(tmpList.contains(subject)) {
				System.out.println("과목 성적이 이미 등록되어있습니다.");
				return;
			}
		...
		private Subject inputSubject() {
			Subject subject = inputRequiredject();
			System.out.print("중간 : ");
			int midterm = nextInt();
			System.out.print("기말 : ");
			int finals = nextInt();
		...
		private void updateSubjectScore() {
			...
			Subject subject = inputRequiredject();
			//위의 두 요구사항을 전부 만족하는 코드
			
			//과목이 과목리스트에 없으면 안내문구 출력 후 종료
			if(!sublist.contains(subject.getName())) {
				System.out.println("등록되지 않은 과목이어서 검색할 수 없습니다.");
				return;
			}
						
			//과목 객체가 학생 성적 리스트에 없으면 안내문구 출력 후 종료
			if(subjects.contains(subject)) {
				System.out.println("등록되지 않은 성적이어서 검색할 수 없습니다.");
				return;
			}
			//중간, 기말, 수행평가를 입력
			subject = inputRequiredject();
			System.out.print("중간 : ");
			int midterm = nextInt();
			System.out.print("기말 : ");
			int finals = nextInt();
			...
			//리스트에서 과목객체를 제거
			//객체를 제거하는 이유는 제거하지 않으면
			//subject에서 해당 과목이 볓번에 있는지 확인해서 해당 과목 정보를 가져오고
	...
	public Subject inputRequiredject() {
		System.out.print("과목 : ");
		scan.nextLine();
		String name = scan.nextLine();
		System.out.print("학년 : ");
	...
	private void runSubjectMenu(int menu) {
		...
		private void insertSubject() {
			...
			//과목 리스트에 등록된 과목인지 확인해서 등록되어 있으면 안내문구 출력
		...
		private void deleteSubject() {
			...
			if(sublist.remove(sub)) {
				System.out.println("과목이 삭제되었습니다");
				return;
			}
			//실패하면 알림문구 출력
			System.out.println("삭제하려는 과목이 없거나 잘못 입력하였습니다");
		}
