...
public class PostServiceImp implements PostService {
	...
	public boolean deleteCommunity(String name) {
		...
		//dao에게 커뮤니티명을 주면서 삭제 요청 후 삭제 여부를 반환
		return postDao.updateCommunity(name);
	}
	...
	public boolean insertPost(PostVO post) {
		//게시글VO null체크해서, 문제가 있을 시 false를 리턴
		if(post == null) {
			return false;
		}
		//제목 null체크, 빈문자열 체크 후 문제가 있을 시 false를 리턴
		if(!checkString(post.getPo_title())) {
			return false;
		}
		//내용 null체크, 빈문자열 체크 후 문제가 있을 시 false를 리턴
		if(!checkString(post.getPo_content())) {
			return false;
		}
		//다오에게 게시글 VO를 주면서 게시글 등록 요청 후 성공여부 반환
		try {
