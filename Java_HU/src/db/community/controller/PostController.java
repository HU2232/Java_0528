...
public class PostController {
	...
	public void deleteCommunity() {
		...
		//서비스에게 커뮤니티명을 주고 삭제하라고 요청 후 성공하면 커뮤니티 삭제 성고
		PrintController.printBar();
	...
	public void insertPost(String id) {
		...
		//서비스에게 게시글VO를 주면서 게시글을 등록 요청 후 성공 여부 출력
		PrintController.printBar();
	...
	public void printPostList(Criteria cri) {
		//서비스에게 페이지 정보(커뮤니티 번호, 검색어)를 주면서 게시글 리스트를 요청
		List<PostVO> list = null;
		try {
			list = postService.getPostList(cri);
		}catch (Exception e) {
			PrintController.printBar();
			//위에서 예외가 발생할 경우 잘못된 커뮤니티 번호입니다 라고 출력
			System.out.println("없는 커뮤니티입니다");
			return;
		}
		//가져온 게시글 리스트의 길이가 0이면 "등록된 게시글이 없습니다" 출력 후 종료
		if(list.size() == 0) {
			System.out.println("등록된 게시글이 없습니다");
			return;
		}
		//가져온 게시글 리스트를 반복문을 이용해서 출력함
		//단, postVO의 toString을 오버라이딩
	...
	public PostVO printPostDetail() {
		...
		//서비스에게 조회하려는 게시글의 조회수 증가를 요청
		postService.updatePostView(poNum);
		//서비스에게 게시글 번호를 주면서 게시글을 요청
	...
	public boolean updatePost(int po_num) {
		...
		scan.nextLine();
		String newCon = scan.nextLine();
		//게시글 번호, 새 제목, 내용을 이용하여 게시글 VO를 생성
		PostVO post = new PostVO(po_num, newName, newCon);
		//서비스에게 게시글VO를 주면서 수정하라고 요청 후 수정 여부를 반환
		return postService.updatePost(post);
	}

	public void insertComment(PostVO post, String id) {
		//post가 null이면 댓글을 추가할 수 없습니다를 출력하고 종료
		if(post == null) {
		...
		scan.nextLine();
		String content = scan.nextLine();
		//게시글 번호, 댓글 내용, 작성자를 이용하여 댓글 vo를 생성하고
		CommentVO comment = new CommentVO(post.getPo_num(), content, id);
		//서비스에게 댓글VO를 주면서 추가 요청후 성공하면 댓글 추가 성공을 출력
		if(postService.insertCommnet(comment)) {
			System.out.println("댓글을 추가하였습니다");
		//실패하면 댓글 추가 실패를 출력
		}else {
			System.out.println("댓글 추가에 실패하였습니다");
		}
	}
